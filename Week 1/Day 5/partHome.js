...
Group.list_of_users = [Users[0]];
Groups[0] = Object.assign({},Group);

Group.group_name = 'BBB';
Group.list_of_users = [Users[1]];
...
function dealWithUserDELETE(answer) {
    ...
    Groups.forEach(function (group) {
        if (group.list_of_users.find(o => o.user_name === answer)) {
            chosenGroup = group;
            chosenUser = userToDelete;
            checkUserInGroupTOREMOVE(chosenUser.user_name, true);
        }
    });
...
function checkUserInGroupTOREMOVE(answer, check) {
    ...
    if (check || chosenGroup.list_of_users.find(o => o.user_name === answer)){
